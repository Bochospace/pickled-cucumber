...
const setup = (httpFn, { compare, getCtx, Given, setCtx, Then, When }) => {
    ...
    const withStatusCode = (status) => `${status} (${STATUS_CODES[status] || 'Unknown'})`;
    ...
    Then('store the response payload at ([\\w_.-]+) in {variable}', (path, id) => setCtx(id, getDeep(getResponseBody(false), path)));
    Then('extract the response location query string argument (.+) into {variable}', (arg, id) => setCtx(id, new URL(getResponse().headers.location).searchParams.get(arg)));
};
